...
class TaskStore:
    ...
    def create_task(self, task_id: str, magnet: str, sample_points: list[int], timeout: int):
        now = datetime.now(timezone.utc).isoformat()
        conn = self._get_conn()
        conn.execute(
            "INSERT INTO tasks (task_id, magnet, sample_points, timeout, status, created_at, updated_at) "
            "VALUES (?, ?, ?, ?, 'queued', ?, ?)",
            (task_id, magnet, json.dumps(sample_points), timeout, now, now),
        )
    ...
    def get_task(self, task_id: str) -> dict | None:
        ...
        for s in snapshots:
            snap = dict(s)
            if snap["filename"]:
                snap["url"] = f"/snapshots/{task_id}/{snap['filename']}"
            else:
                snap["url"] = None
